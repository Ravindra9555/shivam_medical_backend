...
const createAppointment = AsyncHandler(async (req, res) => {
    ...
    const appointment = await Appointment.create({ doctorId, patientId, date, time, comments });
...
const getAllAppointments = AsyncHandler(async (req, res) => {
    ...
    const appointments = await Appointment.find().populate('doctorId', 'name email specialization').populate('patientId', 'name email');
...
const getAllAppointmentbyUserId = AsyncHandler(async (req, res) => {
    ...
    if (!appointments || appointments.length === 0) {
        return res.status(404).json(new ApiResponse(404, "No appointments found for this user", []));
    }
...
const getAllAppointmentsByDate = AsyncHandler(async (req, res) => {
 ...
 console.log(
    ...
    if (isNaN(startDate.getTime())) {
        throw new ApiError(400, "Invalid date format. Please provide a valid date.");
    }
    ...
    // Check if any appointments were found
    if (!appointments || appointments.length === 0) {
        return res.status(404).json(new ApiResponse(404, "No appointments found for this date", []));
    }
...
export { getAllAppointments, createAppointment,getAllAppointmentbyUserId, getAllAppointmentsByDate };
